...
const ProjectCard = ({ project }) => {
  ...
  return (
    ...
    >
      <div className="relative bg-white dark:bg-neutral-900 rounded-2xl overflow-hidden border border-neutral-200 dark:border-neutral-800 hover:border-brand-400/50 dark:hover:border-brand-600/50 hover:shadow-xl transition-all duration-300"
    >
        {/* Project Image */}
          ...
          {showGitHubModal && (
            ...
            >
              <motion.div
                ...
                className="glass-effect rounded-3xl p-8 max-w-md w-full shadow-glass-lg border border-brand-200/20 dark:border-brand-800/20"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-2xl font-display font-bold text-neutral-900 dark:text-white">{t('projects.accessTitle')}</h3>
                  <button
                    onClick={() => setShowGitHubModal(false)}
                ...
                <div className="flex justify-end space-x-3">
                  <button
                    onClick={() => setShowGitHubModal(false)}
                    className="px-6 py-3 glass-effect border border-brand-200/30 dark:border-brand-800/30 text-neutral-800 dark:text-neutral-200 rounded-2xl hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors font-medium"
                  >
                    {t('projects.close')}
                  </button>
                  <a
                    ...
                    className="px-6 py-3 bg-gradient-to-r from-brand-500 to-accent-cyan text-white rounded-2xl hover:shadow-lg hover:shadow-brand-500/30 transition-all font-semibold"
                  >
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          {project.tags
            ...
            .map(tag => (
              <span
                key={tag}
                className="px-3 py-1 text-xs font-medium bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 rounded-md"
              >
                {tag}
              </span>
            ))
...
return (
    <section id="projects" className="py-24 bg-neutral-50 dark:bg-neutral-900 transition-colors duration-300">
            ...
            <motion.button
              ...
              className={`px-5 py-2.5 rounded-2xl text-sm font-medium transition-all ${
                selectedTag === tag
                  ...
                  : 'glass-effect border border-brand-200/30 dark:border-brand-800/30 text-neutral-700 dark:text-neutral-300 hover:border-brand-400/50'
              }`}
